...
execute if entity @s[tag=trap_bees,limit=1] at @s run data modify entity @s AngryAt set from entity @e[type=#adventure:interactables/bee_attackable,limit=1,sort=nearest,distance=..10] UUID 
execute if entity @s[nbt={HasStung:1b},limit=1] at @s run data modify entity @s HasStung set value 0
